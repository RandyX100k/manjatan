...
class TestAccountReportsTaxReminder(TestAccountReportsCommon):
    ...
    def test_tax_closing_activity_reminder_reset_on_periodicity_change(self):
        ...
        """
        us_report = self.env.ref('l10n_us_account.tax_report')
        old_activity = self.env.company._get_tax_closing_reminder_activity(us_report.id, fields.Date.from_string(self.options['date']['date_to']))

        self.env.company.account_tax_periodicity = 'year'
        _dummy, period_end = self.env.company._get_tax_closing_period_boundaries(fields.Date.today(), us_report)
        new_activity = self.env.company._get_tax_closing_reminder_activity(us_report.id, period_end)
